...
Functions `fs.dosage, fst.dosage`  and `fis.dosage` give estimates of population specific $F_{ST}^i$ and $F_{IS}^i$ for dosage data, `fs.dosage` also outputs the matrix of $F_{ST}^{XY}$, as defined in [Weir and Goudet (2017)](https://www.genetics.org/content/206/4/2085) and [Weir and Hill (2002)](https://www.annualreviews.org/doi/abs/10.1146/annurev.genet.36.050802.093940). We use the `gtrunchier` dataset to illustrate the output of `fs.dosage`. We first need to convert `gtrunchier` to a dosage format  via `fstat2dos` 
...
A function to detect sex-biased dispersal, `sexbias.test` based on [Goudet et al. (2002)](https://onlinelibrary.wiley.com/doi/10.1046/j.1365-294X.2002.01496.x/abstract) has been implemented. 
To illustrate its use, load the *Crocidura russula* data set. It consists of the genotypes and sexes of 140 shrews studied by [Favre et al. (1997)](https://rspb.royalsocietypublishing.org/content/264/1378/127). In this species, mark -recapture showed an excess of dispersal from females, an unusual pattern in mammals. This is confirmed using genetic data: 
